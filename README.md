...
**可直接双击./markdown2word 文件夹下的 exe 程序运行本程序，简单快捷**
...
- setting.json中的
  ...
  - markdown_word_wrap（Markdown 输入框是否自动换行），默认是 False
- GUI 顶部可填写保存文件名，可填 `xxx` 或 `xxx.docx`；不填则继续按 `title_chars` 自动生成
- 遇到同名文件时自动保存为 `xxx(1).docx`、`xxx(2).docx`
- `Ctrl+S` 可直接运行并生成 Word
- Markdown 输入框支持自动换行开关，关闭后显示横向滚动条
...
## 后续调优历程
  ...
  - 2026年6月26日——将公式中的 aligned / align / split 转成 Word OMML 矩阵，多列公式会对齐，支持markdown的>块引用格式，支持URL图片的插入
  - 2026年6月28日——对于带编号的公式支持编号右对齐，同时公式居中显示；支持 word 多种样式的初始化；支持（嵌套）无序列表、完成/待办、--- 水平线；优化∑公式的渲染；设置纸张大小为A4纸；一行多公式\quad正确显示
  - 2026年7月11日——再次修复一行多公式\quad正确显示；\begin{array}{rl} ... \end{array} \quad (n) 会先转成等号对齐结构，再生成 Word 矩阵列对齐；\dagger 匕首符正确显示为 †；双重求和公式对第一个求和的空操作数补一个不可见的零宽占位字符 U+2060，防止 Word 生成占位框。
  - 2026年8月19日——优化 \underbrace、\overbrace 大括号的生成效果
  - 2026年8月29日——没有空行分隔的内容解析成同一个 &lt;p&gt; ,按整个 &lt;p&gt; 判断样式变为简单文本段落的按行渲染逻辑
  - 2026年9月15日——GUI 顶部增加保存文件名输入框，运行按钮移到顶部，支持 Ctrl+S 运行；正文首行缩进改为“缩进/顶格”互斥选择；导出文件名取消时间戳，同名文件自动追加序号；题注不加粗；正文和引用块两端对齐；\widehat公式渲染修正
  - 2026年9月16日——Markdown 输入框增加自动换行开关，关闭自动换行时显示横向滚动条
...
### 双击 markdown2word.exe 程序运行

./markdown2word 文件夹下有已经打包好的 exe 程序，直接双击运行已经打包好的程序，无需配置python环境。这种方式最简单直接，同时可鼠标右键创建快捷方式放到桌面供以后使用，方便快捷。
...
这种方式步骤更少，适合简单项目的统筹 base 管理，但会把本项目依赖安装到 `base` 环境中。如果希望环境更干净，建议使用下面的专用环境方式。
